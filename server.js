...
app.post('/api/login', (req, res) => {
  ...
  if (user) {
    ...
    res.json({ ok: true, user: { username: user.username, canPrint: user.canPrint } });
  } else {
...
app.get('/get-settings', (req, res) => {
  // Users can read settings for calculation
...
function loadMergedSettings() {
  if (fs.existsSync(settingsPath)) {
    try {
      ...
      if (!savedSettings.inverters_load_rules && DEFAULT_SETTINGS.inverters_load_rules) {
        mergedSettings.inverters_load_rules = DEFAULT_SETTINGS.inverters_load_rules;
      }
      if (!savedSettings.prices && DEFAULT_SETTINGS.prices) {
        mergedSettings.prices = DEFAULT_SETTINGS.prices;
      }
      if (!savedSettings.panel_names && DEFAULT_SETTINGS.panel_names) {
